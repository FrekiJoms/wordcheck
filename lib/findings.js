 ...
 * Resets ID counter per scan so IDs are stable (F-0001..).
 * Deduplicates identical findings per paragraph.
 */
function buildFindings(scanResult) {
  ...
  for (const para of scanResult.paragraphs) {
    ...
    const paraFindings = flagsToFindings(para);
    findings.push(...paraFindings);
  }
  // Deduplicate by paragraph + title (defensive: scanner should already dedup overlapping phrases)
  const seen = new Set();
  const deduped = [];
  for (const f of findings) {
    const key = f.paragraphIndex + "::" + f.title;
    if (!seen.has(key)) {
      seen.add(key);
      deduped.push(f);
    }
