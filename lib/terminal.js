...
class Terminal {
  ...
  _drawContent() {
    // Skip content redraw when any modal is open — modals own the area
  ...
  _drawProviderModal() {
    ...
    if (canScrollUp) {
      this._write(ansi.cursorTo(row, startCol) + ansi.eraseLine);
      this._write(chalk.bgHex("#1e1e2e")(" " + C.cyan("  \u25B2 more above") + " ".repeat(w - 15) + " "));
      row++;
      maxItems--;
    }
    ...
    if (canScrollDown && rendered < maxItems) {
      this._write(ansi.cursorTo(row, startCol) + ansi.eraseLine);
      this._write(chalk.bgHex("#1e1e2e")(" " + C.cyan("  \u25BC more below") + " ".repeat(w - 15) + " "));
      row++;
    }
      ...
      if (hasGroups && model.group && model.group !== lastGroup) {
        ...
        if (itemIdx >= scrollOff && rendered < maxItems - 1) {
          ...
          const groupBadge = model.group === "Free" ? C.green(model.group) :
                            ...
                            C.dim(model.group);
          this._write(chalk.bgHex("#1e1e2e")(" " + C.dim("  ") + groupBadge + C.dim(" models") + " ".repeat(Math.max(0, w - 4 - model.group.length - 8)) + " "));
          row++;
          rendered++;
        }
      }
  ...
  _drawToolProgress() {
    ...
    for (let i = tp.scrollOffset; i < tp.tools.length && rendered < maxVis; i++) {
      ...
      this._write(bg(" " + statusIcon + nameStr + " ".repeat(pad) + " "));
      row++;
      rendered++;
    }
  ...
  _drawSettingsModal() {
    ...
    // Count line
    this._write(ansi.cursorTo(row, startCol) + ansi.eraseLine);
    this._write(chalk.bgHex("#1e1e2e")(" " + C.dim(`  ${sm.fields.length} settings`) + " ".repeat(Math.max(0, w - 2 - 4 - String(sm.fields.length).length - 9)) + " "));
    row++;
    ...
    this._write(chalk.bgHex("#1e1e2e")(" " + hint + " ".repeat(Math.max(0, w - 2 - 44)) + " "));
  }
