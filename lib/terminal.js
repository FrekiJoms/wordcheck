...
class ModelModal {
  constructor() {
    ...
    this._modalW      = 0;
    this.loading      = false;
    this._visibleItems = 0; // max items that fit on screen
  }
...
class Terminal {
  constructor() {
    ...
    this.toolProgress = {
      visible: false,
      tools: [],         // [{ name, status: "pending"|"running"|"done"|"error" }]
      currentIndex: 0,
      scrollOffset: 0,
    };
  ...
  _drawContent() {
    // When any modal is open, draw dim overlay instead of content
    if (this._anyModalOpen()) {
  ...
  _drawModalOverlay() {
    ...
    for (let i = 0; i < visibleRows; i++) {
      ...
      if (i < contentSlice.length) {
        ...
        this._write(dimBg(dimFg(stripped)));
      } else {
  ...
  _drawProviderModal() {
    ...
    const contentLens = pm.filtered.map((e) => {
      if (e.sectionSep) return e.label.length;
      return 2 + (e.label || "").length + (e.badge ? e.badge.length + 1 : 0) + (e.desc ? e.desc.length + 2 : 0);
    });
    const w        = this._calcModalWidth([...contentLens, "PROVIDERS 99/99".length]);
    ...
    const countStr = ` ${pm.filtered.length}/${pm.entries.length}`;
    const escStr = "esc";
    const titlePad = w - 2 - titleStr.length - countStr.length - escStr.length;
    this._write(
      chalk.bgHex("#1e1e2e")(
        " ".repeat(startCol - 1) +
        " " + C.white.bold(titleStr) + C.dim(countStr) +
        " ".repeat(Math.max(1, titlePad)) + C.dim(escStr) + " " +
        " ".repeat(Math.max(0, this.width - startCol - w))
      )
    );
    row++;
    ...
    this._write(
      chalk.bgHex("#1e1e2e")(
        " ".repeat(startCol - 1) +
        " " + C.cyan("S") + C.dim("earch") + C.dim(" ") + searchDisplay + " ".repeat(Math.max(0, w - 8 - pm.search.length)) + " " +
        " ".repeat(Math.max(0, this.width - startCol - w))
      )
    );
    row++;
    ...
    // Separator
    this._write(ansi.cursorTo(row, 1) + ansi.eraseLine);
    this._write(chalk.bgHex("#1e1e2e")(" ".repeat(startCol - 1) + " " + C.dim("\u2500".repeat(w - 2)) + " " + " ".repeat(Math.max(0, this.width - startCol - w))));
    row++;
    ...
    if (canScrollUp) {
      this._write(ansi.cursorTo(row, 1) + ansi.eraseLine);
      this._write(chalk.bgHex("#1e1e2e")(" ".repeat(startCol - 1) + " " + C.cyan("  \u25B2 more above") + " ".repeat(w - 15) + " " + " ".repeat(Math.max(0, this.width - startCol - w))));
      row++;
      maxItems--;
    }
    ...
    for (let fi = 0; fi < pm.filtered.length; fi++) {
      ...
      if (entry.sectionSep) {
        this._write(
          chalk.bgHex("#1e1e2e")(
            " ".repeat(startCol - 1) +
            " " + C.pink.bold(entry.label) + " ".repeat(Math.max(0, w - 2 - entry.label.length)) + " " +
            " ".repeat(Math.max(0, this.width - startCol - w))
          )
        );
      } else {
    ...
    if (canScrollDown && rendered < maxItems) {
      this._write(ansi.cursorTo(row, 1) + ansi.eraseLine);
      this._write(chalk.bgHex("#1e1e2e")(" ".repeat(startCol - 1) + " " + C.cyan("  \u25BC more below") + " ".repeat(w - 15) + " " + " ".repeat(Math.max(0, this.width - startCol - w))));
      row++;
    }
  ...
  _drawModelModal() {
    ...
    const contentLens = mm.filtered.map((m) => {
      return 2 + m.id.length + (m.group ? m.group.length + 3 : 0) + (m.desc ? m.desc.length + 2 : 0);
    });
    const titleLen = 5 + (mm.providerLabel ? mm.providerLabel.length + 2 : 0) + " 99/99".length;
    const w        = this._calcModalWidth([...contentLens, titleLen, 22]);
    const startCol = Math.max(1, Math.floor((this.width - w) / 2));
    const startRow = this.contentRowStart;
    let   maxItems = this.contentHeight - 7;
    ...
    const escStr = "esc";
    const titlePad = w - 2 - titleStr.length - mm.providerLabel.length - countStr.length - escStr.length;
    this._write(
      chalk.bgHex("#1e1e2e")(
        " ".repeat(startCol - 1) +
        " " + C.white.bold(titleStr) + C.dim("  " + mm.providerLabel) + C.dim(countStr) +
        " ".repeat(Math.max(1, titlePad)) + C.dim(escStr) + " " +
        " ".repeat(Math.max(0, this.width - startCol - w))
      )
    );
    ...
    this._write(
      chalk.bgHex("#1e1e2e")(
        " ".repeat(startCol - 1) +
        " " + C.cyan("S") + C.dim("earch") + C.dim(" ") + searchDisplay + " ".repeat(Math.max(0, w - 8 - mm.search.length)) + " " +
        " ".repeat(Math.max(0, this.width - startCol - w))
      )
    );
    row++;
    ...
    if (mm.loading && mm.filtered.length === 0) {
      this._write(ansi.cursorTo(row, 1) + ansi.eraseLine);
      this._write(chalk.bgHex("#1e1e2e")(" ".repeat(startCol - 1) + " " + C.dim("  Loading models...") + " ".repeat(w - 22) + " " + " ".repeat(Math.max(0, this.width - startCol - w))));
      row++;
    }

    // Models — scrollable with group headers
    mm._itemRows = [];
    ...
    for (let fi = 0; fi < mm.filtered.length; fi++) {
      ...
      if (hasGroups && model.group && model.group !== lastGroup) {
        ...
        // Only show header if we're past scroll offset
        if (itemIdx >= scrollOff && rendered < maxItems - 1) {
          ...
          const groupBadge = model.group === "Free" ? C.green(model.group) :
                            ...
                            C.dim(model.group);
          this._write(chalk.bgHex("#1e1e2e")(" ".repeat(startCol - 1) + " " + C.dim("  ") + groupBadge + C.dim(" models") + " ".repeat(Math.max(0, w - 4 - model.group.length - 8)) + " " + " ".repeat(Math.max(0, this.width - startCol - w))));
          row++;
          rendered++;
        }
      }
      ...
      const contentLen = (isActive ? 2 : 2) + labelStr.length + (model.group ? model.group.length + 3 : 0) + (model.desc ? model.desc.length + 2 : 0);
      const pad = Math.max(0, w - 2 - contentLen);
      const bg = isHovered ? chalk.bgHex("#313244") : chalk.bgHex("#1e1e2e");
  ...
  // -----------------------------------------------------------------------
  // Tool progress overlay — scrollable list of tools being executed
  // -----------------------------------------------------------------------
  ...
  updateToolProgress(currentIndex) {
    ...
    if (currentIndex > 0 && currentIndex <= tp.tools.length) {
      tp.tools[currentIndex - 1].status = "done";
    }
    // Mark current as running
    if (currentIndex < tp.tools.length) {
      tp.tools[currentIndex].status = "running";
      tp.currentIndex = currentIndex;
    }
    // Auto-scroll
    const maxVis = Math.min(tp.tools.length, this.contentHeight - 4);
    if (currentIndex >= tp.scrollOffset + maxVis) {
      tp.scrollOffset = currentIndex - maxVis + 1;
    }
    this._dirty.toolProgress = true;
    this._dirty.content = true;
    this._dirty.input = true;
  ...
  _drawToolProgress() {
    ...
    const w = this._calcModalWidth([...contentLens, 20]);
    const startCol = Math.max(1, Math.floor((this.width - w) / 2));
    const startRow = this.contentRowStart;
    const maxVis = Math.min(tp.tools.length, this.contentHeight - 4);
    let row = startRow;
    ...
    this._write(
      chalk.bgHex("#1e1e2e")(
        " ".repeat(startCol - 1) +
        " " + C.white.bold("TOOLS") + " " + C.dim(`${tp.currentIndex + 1}/${tp.tools.length}`) +
        " ".repeat(Math.max(1, w - 2 - 5 - String(tp.tools.length).length * 2 - 3)) + C.dim("esc") + " " +
        " ".repeat(Math.max(0, this.width - startCol - w))
      )
    );
    row++;
    ...
    for (let i = tp.scrollOffset; i < tp.tools.length && rendered < maxVis; i++) {
      ...
      if (tool.status === "done") {
        statusIcon = C.green(" \u2713 ");
        nameStr = C.dim(tool.name);
      } else if (tool.status === "error") {
        statusIcon = C.red(" \u2717 ");
        nameStr = C.red(tool.name);
      } else if (tool.status === "running") {
        statusIcon = C.cyan(" \u25B6 ");
        nameStr = C.white.bold(tool.name);
      } else {
        statusIcon = C.dim("   ");
        nameStr = C.dim(tool.name);
      }
      ...
      this._write(bg(" ".repeat(startCol - 1) + " " + statusIcon + nameStr + " ".repeat(pad) + " " + " ".repeat(Math.max(0, this.width - startCol - w))));
      row++;
      rendered++;
    }
  ...
  _drawSettingsModal() {
    ...
    const contentLens = sm.fields.map((f) => {
      ...
      return 4 + f.label.length + val.length;
    });
    const w        = this._calcModalWidth([...contentLens, "SETTINGS".length + 4, 50]);
    const startCol = Math.max(1, Math.floor((this.width - w) / 2));
    const startRow = this.contentRowStart;
    ...
    for (let i = 0; i < sm.fields.length; i++) {
      ...
      this._write(bg(" ".repeat(startCol - 1) + " " + numStr + labelRendered + val + " " + " ".repeat(Math.max(0, this.width - startCol - w))));
    ...
    // Count line
    this._write(ansi.cursorTo(row, 1) + ansi.eraseLine);
    this._write(chalk.bgHex("#1e1e2e")(" ".repeat(startCol - 1) + " " + C.dim(`  ${sm.fields.length} settings`) + " ".repeat(Math.max(0, w - 2 - 4 - String(sm.fields.length).length - 9)) + " " + " ".repeat(Math.max(0, this.width - startCol - w))));
    row++;
    ...
    this._write(chalk.bgHex("#1e1e2e")(" ".repeat(startCol - 1) + " " + hint + " ".repeat(Math.max(0, w - 2 - 44)) + " " + " ".repeat(Math.max(0, this.width - startCol - w))));
  }
  ...
  _drawModal() {
    ...
    const w          = this._calcModalWidth([...m.lines.map((l) => l.length), m.title.length]);
    ...
    this._write(chalk.bgHex("#1e1e2e")(leftPad + " " + C.pink.bold(m.title) + " ".repeat(Math.max(0, w - 2 - m.title.length)) + " " + rightPad));

    this._write(ansi.cursorTo(startRow + 1, 1) + ansi.eraseLine);
    this._write(chalk.bgHex("#1e1e2e")(leftPad + " " + C.dim("\u2500".repeat(w - 2)) + " " + rightPad));
    ...
    for (let i = 0; i < maxVisible; i++) {
      ...
      this._write(
        bg(leftPad + " " + (sel ? C.pink("\u25B6 ") : "  ") + (sel ? C.white.bold(item) : C.dim(item)) + " " + rightPad)
      );
    }
    ...
    if (hintRow <= this.statusRow - 2) {
      ...
      this._write(chalk.bgHex("#1e1e2e")(leftPad + " " + C.dim("\u2191\u2193 navigate  Enter select  Esc cancel") + " " + rightPad));
    }
  }
      ...
      if (ch === "\x1B") {
        ...
        if (seq.startsWith("\x1B[<")) {
          ...
          if (m) {
            ...
            // scroll wheel scrolls the provider list
      ...
      if (ch === "\x1B") {
        ...
        if (seq.startsWith("\x1B[<")) {
          ...
          if (m) {
            ...
            // scroll wheel scrolls the model list
      ...
      if (ch === "\x1B") {
        ...
        if (seq.startsWith("\x1B[<")) {
          ...
          if (m) {
            ...
            // scroll wheel — no-op for settings (small list)
            i += seq.indexOf("M", 3) !== -1 ? seq.indexOf("M", 3) + 1 : seq.indexOf("m", 3) + 1;
            continue;
          }
        }
