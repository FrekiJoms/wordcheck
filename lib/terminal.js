...
class Terminal {
  ...
  _drawModal() {
    ...
    // Overlay: dim the content behind the modal
    for (let i = 0; i < this.contentHeight; i++) {
      const row = this.contentRowStart + i;
      if (row >= startRow && row < startRow + modalHeight) continue;
      this._write(ansi.cursorTo(row, 1) + ansi.eraseLine + C.dim("\u2591".repeat(this.width - 2)));
    }
...
module.exports = { Terminal, C, ansi, wrapText };
