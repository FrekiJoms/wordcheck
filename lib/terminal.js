...
class Terminal {
  ...
  _drawModelModal() {
    ...
    if (mm.models.length > 0) {
      this._write(ansi.cursorTo(row, startCol) + ansi.eraseLine);
      const scrollInfo = canScrollUp || canScrollDown ? ` (${scrollOff + 1}-${Math.min(scrollOff + rendered, mm.filtered.length)})` : "";
      this._write(chalk.bgHex("#1e1e2e")(" " + C.dim(`  ${mm.filtered.length}/${mm.models.length} models${scrollInfo}`) + " ".repeat(Math.max(0, w - 4 - String(mm.models.length).length * 2 - scrollInfo.length)) + " "));
      row++;
    }
