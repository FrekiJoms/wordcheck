...
class SettingsModal {
  constructor() {
    ...
    this.editing        = -1;
    this.editBuffer     = "";
    this.editCursorPos  = 0;   // FIX: track cursor position in edit buffer
    this.onSave         = null;
    this.onCancel       = null;
...
class Terminal {
  constructor() {
    ...
    this.toolProgress = {
      visible: false, tools: [], currentIndex: 0, scrollOffset: 0,
    };
  ...
  _drawProviderModal() {
    ...
    // FIX: compute stable width from ALL items once, then reuse it every render.
    // Previously width was recomputed from pm.filtered (the visible slice), causing
    // it to fluctuate as items scrolled in/out, producing modal resize flicker.
    if (!pm._stableW) {
    ...
    let   maxItems = this.contentHeight - 5;
    ...
    // Items — scrollable
    pm._visibleItems = maxItems;
    let itemIdx  = 0;
    let rendered = 0;
    const scrollOff    = pm.scrollOffset || 0;
    const canScrollUp  = scrollOff > 0;
    const canScrollDown = scrollOff + maxItems < pm.filtered.length;
  ...
  _drawModelModal() {
    ...
    // FIX: compute stable width from ALL models once on first draw, then reuse.
    // Previously width was computed from mm.filtered (the current search slice),
    // so scrolling — which changes which items are visible — would change the
    // computed max length and cause the modal to resize/flicker on every frame.
    if (!mm._stableW) {
    ...
    const startCol = Math.max(2, Math.floor((this.width - w) / 2) + 1);
    const startRow = this.contentRowStart;
    let   maxItems = this.contentHeight - 6;
    ...
    const canScrollUp  = scrollOff > 0;
    const canScrollDown = scrollOff + maxItems < mm.filtered.length;
    ...
    for (let fi = 0; fi < mm.filtered.length; fi++) {
      ...
      if (hasGroups && model.group && model.group !== lastGroup) {
        ...
        if (itemIdx >= scrollOff && rendered < maxItems - 1) {
          this._write(ansi.cursorTo(row, 1) + ansi.eraseLine);
          const groupBadge = model.group === "Free"    ? C.green(model.group) :
                             model.group === "Go"      ? C.cyan(model.group) :
                             model.group === "Premium" ? C.yellow(model.group) :
                             C.dim(model.group);
  ...
  _drawToolProgress() {
    ...
    const contentLens = tp.tools.map((t) => 3 + t.name.length);
    const w        = this._calcModalWidth([...contentLens, 20], 6);
    const startCol = Math.max(2, Math.floor((this.width - w) / 2) + 1);
    const startRow = this.contentRowStart;
    const maxVis   = Math.min(tp.tools.length, this.contentHeight - 4);
    let row = startRow;
    ...
    let rendered = 0;
    for (let i = tp.scrollOffset; i < tp.tools.length && rendered < maxVis; i++) {
    ...
    if (tp.scrollOffset + maxVis < tp.tools.length) {
      this._write(ansi.cursorTo(row, 1) + ansi.eraseLine);
      this._write(this._modalRow(startCol, w, " " + C.dim("  \u25BC more below")));
      row++;
    }
  ...
  _drawSettingsModal() {
    ...
    // Compute width from field label + value lengths
    const contentLens = sm.fields.map((f) => {
      const val = f.masked ? (f.raw ? "*".repeat(Math.min(f.raw.length, 16)) + "..." : "(not set)") : (f.value || "");
      return 4 + 14 + val.length + 4; // num + label padded + value + margins
    });
    const w        = this._calcModalWidth([...contentLens, 20, 55], 6);
    const startCol = Math.max(2, Math.floor((this.width - w) / 2) + 1);
    const startRow = this.contentRowStart;
  ...
  _drawModal() {
    ...
    const w          = this._calcModalWidth([...m.lines.map((l) => l.length), m.title.length], 6);
