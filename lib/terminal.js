...
class ProviderModal {
  constructor() {
    ...
    this.allItems     = [];
    this.filtered     = [];
    this.entries      = []; // flat list for count display
    this.hovered      = 0;
    this.scrollOffset = 0;
    this.activeId     = null;
    this.escHint      = "esc";
    ...
    this._startRow    = 0;
    this._modalW      = 0;
    this._stableW     = 0;   // FIX: locked width, computed once on open
  ...
  open({ title, sections, activeId, onSelect, onCancel, escHint = "esc" }) {
    ...
    // FIX: compute stable width from ALL items once on open, never again
    this._stableW = 0;
  }
  ...
  _hoveredFlatIdx() { return this._itemsOnly().findIndex((x) => this._itemHoverIdx(x.i) === this.hovered); }
...
class ModelModal {
  ...
  open({ models, activeModel, providerLabel, onSelect, onCancel, onProvider }) {
    ...
    // FIX: reset stable width so it gets recomputed on first draw with all models
    this._stableW = 0;
  }
  ...
  setModels(models) {
    this.models = models;
    this._applyFilter();
    // FIX: reset stable width when model list is populated (e.g. after async load)
    this._stableW = 0;
  }
  ...
  up() {
    if (this.hovered > 0) {
      this.hovered--;
      if (this.hovered < this.scrollOffset) this.scrollOffset = this.hovered;
    }
  }
  down() {
  ...
  scrollUp(n = 3)  { this.scrollOffset = Math.max(0, this.scrollOffset - n); this.hovered = Math.max(this.hovered, this.scrollOffset); }
  scrollDown(n = 3) {
    const maxVis = this._visibleItems || 10;
    const maxOff = Math.max(0, this.filtered.length - maxVis);
    this.scrollOffset = Math.min(maxOff, this.scrollOffset + n);
    this.hovered = Math.min(this.hovered, this.scrollOffset + maxVis - 1);
  }
...
class SettingsModal {
  constructor() {
    ...
    this._startRow      = 0;
    this._modalW        = 0;
    this._stableW       = 0; // locked width from all fields
  }
  ...
  open({ fields, onSave, onCancel, onOpenProvider }) {
    ...
    this._stableW       = 0; // recompute on next draw from all fields
  }
  ...
  startEdit(idx) {
    ...
    this.editCursorPos = this.editBuffer.length; // cursor at end
  }
  ...
  // FIX: proper cursor-aware text editing
  appendChar(ch) {
  ...
  // FIX: Ctrl+A = select all (move to home for edit context)
  selectAll()   { this.editCursorPos = 0; }
  // FIX: Ctrl+V = paste from clipboard (handled in keypress handler)
  pasteText(text) {
  ...
  // FIX: Ctrl+K = clear to end of line
  killToEnd()   {
    if (this.editing < 0) return;
    this.editBuffer = this.editBuffer.slice(0, this.editCursorPos);
  }
  // FIX: Ctrl+U = clear entire line
  killLine()    {
    if (this.editing < 0) return;
    this.editBuffer    = "";
    this.editCursorPos = 0;
  }
  // FIX: Ctrl+W = delete word backwards
  deleteWordBack() {
    if (this.editing < 0 || this.editCursorPos === 0) return;
...
class Terminal {
  constructor() {
    ...
    this.toolProgress = {
      visible: false, tools: [], currentIndex: 0, scrollOffset: 0, _stableW: 0,
    };

    this.modal = {
      visible: false, title: "", lines: [], selected: 0, onSelect: null, onCancel: null, _stableW: 0,
    };
  ...
  // FIX: helper to measure content width against modal boundaries properly
  ...
  // FIX: helper to draw a full-width modal row (no ragged right edge)
  _modalRow(startCol, w, inner) {
    const bgFill = chalk.bgHex("#1e1e2e");
    // inner is pre-styled content; we need to track visible length separately
    const strip  = (s) => s.replace(/\x1B\[[0-9;]*m/g, "");
    const innerVis = strip(inner).length;
    const padRight = Math.max(0, w - innerVis);
    const leftPad  = " ".repeat(startCol - 1);
    const rightPad = " ".repeat(Math.max(0, this.width - startCol - w));
  ...
  // FIX: _drawInput — settings editing input no longer leaks into the AI prompt bar
  // When settings modal is open and editing, show edit state in the dedicated modal
  // field inline (cursor rendered in _drawSettingsModal), input row just shows hint.
  _drawInput() {
    this._write(ansi.cursorTo(this.inputRow, 1) + ansi.eraseLine);

    if (this.settingsModal.visible) {
      // FIX: NEVER render edit buffer in the main input row — this was causing the
      // "input reflected in AI input area" bug. Just show a neutral hint instead.
  ...
  _drawModalOverlay() {
    ...
    const visibleRows = this.contentHeight;
    const contentSlice = this.contentLines.slice(this.scrollOffset, this.scrollOffset + visibleRows);
    for (let i = 0; i < visibleRows; i++) {
      const row = this.contentRowStart + i;
      this._write(ansi.cursorTo(row, 1) + ansi.eraseLine);
    ...
    this._write(ansi.cursorTo(this.statusRow, 1) + ansi.eraseLine);
    const leftV  = `  \u25CF wordcheck  v${VERSION}`;
    const rightV = this.statusText;
    const gap = Math.max(1, this.width - leftV.length - rightV.length - 2);
  ...
  _drawProviderModal() {
    ...
    // Stable width from ALL items (HTML: 420px locked on open, never from filtered slice)
    if (!pm._stableW) {
      const allLens = pm.allItems.map((e) => {
        if (e.sectionSep) return 2 + e.label.length;
        return 4 + (e.label || "").length + (e.badge ? e.badge.length + 1 : 0) + (e.desc ? e.desc.length + 2 : 0);
      });
    ...
    const FIXED_VISIBLE = this.contentHeight - 5; // scrollable rows (HTML: VISIBLE)
    ...
    // Title bar — flex justify-between, border-bottom
    this._write(ansi.cursorTo(row, 1) + ansi.eraseLine);
    const titleStr = "PROVIDERS";
    const countStr = ` ${filteredCount}/${entryCount}`;
    const escStr   = "esc ";
    const titleInner = " " + C.white.bold(titleStr) + C.dim(countStr) + " ".repeat(Math.max(1, w - 1 - titleStr.length - countStr.length - escStr.length)) + C.dim(escStr);
    this._write(this._modalRow(startCol, w, titleInner));
    row++;
    ...
    const searchInner = " " + C.dim("\u2315 ") + searchDisplay + " ".repeat(Math.max(0, w - 3 - searchVis));
    this._write(this._modalRow(startCol, w, searchInner));
    row++;

    // Separator — 0.5px solid #313244
    this._write(ansi.cursorTo(row, 1) + ansi.eraseLine);
    this._write(this._modalRow(startCol, w, " " + C.dim("\u2500".repeat(w - 2))));
    row++;

    // Scrollable content — fixed container to prevent flicker (HTML: modal 420px, VISIBLE rows)
    const scrollOff = pm.scrollOffset || 0;
    const isScrollable = pm.filtered.length > FIXED_VISIBLE;
    const canScrollUp = isScrollable && scrollOff > 0;
    const canScrollDown = isScrollable && scrollOff + FIXED_VISIBLE < pm.filtered.length;
    // Items visible inside fixed container (hints consume rows when scrollable)
    const itemsVisible = isScrollable ? FIXED_VISIBLE - (canScrollUp ? 1 : 0) - (canScrollDown ? 1 : 0) : pm.filtered.length;
    pm._visibleItems = isScrollable ? FIXED_VISIBLE : pm.filtered.length;
    ...
    for (let fi = 0; fi < pm.filtered.length; fi++) {
      const entry = pm.filtered[fi];
      if (fi < scrollOff) { if (!entry.sectionSep) itemIdx++; continue; }
      if (rendered >= itemsVisible) break;

      this._write(ansi.cursorTo(row, 1) + ansi.eraseLine);

      if (entry.sectionSep) {
        const inner = " " + C.pink.bold(" " + entry.label);
    ...
    // Fill remaining rows in fixed container to keep box height stable (prevents flicker)
    if (isScrollable) {
      ...
      // Fill any leftover rows up to FIXED_VISIBLE (should already be filled)
    ...
    // Footer — border-top, fixed at bottom of modal container (HTML: 0.5px solid #313244)
    this._write(ansi.cursorTo(row, 1) + ansi.eraseLine);
    const footerHint = "\u2191\u2193 navigate  Enter select  type to search  Esc cancel";
    this._write(this._modalRow(startCol, w, " " + C.dim(footerHint)));
  }
  ...
  _drawModelModal() {
    ...
    // Stable width from ALL models (HTML: 420px locked on open)
    if (!mm._stableW) {
      const allLens = mm.models.map((m) => {
        return 4 + m.id.length + (m.group ? m.group.length + 3 : 0) + (m.desc ? m.desc.length + 2 : 0);
      });
    ...
    const startCol = Math.max(2, Math.floor((this.width - w) / 2) + 1);
    const startRow = this.contentRowStart;
    const FIXED_VISIBLE = this.contentHeight - 6; // HTML VISIBLE rows
    ...
    // Title — flex justify-between, border-bottom 0.5px #313244
    this._write(ansi.cursorTo(row, 1) + ansi.eraseLine);
    const titleStr  = "MODEL";
    const provStr   = mm.providerLabel ? "  " + mm.providerLabel : "";
    const countStr  = ` ${mm.filtered.length}/${mm.models.length}`;
    const escStr    = "esc ";
    const titlePad  = Math.max(1, w - 1 - titleStr.length - provStr.length - countStr.length - escStr.length);
    const titleInner = " " + C.white.bold(titleStr) + C.dim(provStr) + C.dim(countStr) + " ".repeat(titlePad) + C.dim(escStr);
    this._write(this._modalRow(startCol, w, titleInner));
    row++;
    ...
    // Scrollable list — fixed container (prevents flicker)
    const isScrollable = mm.filtered.length > FIXED_VISIBLE;
    const scrollOff = mm.scrollOffset || 0;
    const canScrollUp = isScrollable && scrollOff > 0;
    const canScrollDown = isScrollable && scrollOff + FIXED_VISIBLE < mm.filtered.length;
    const itemsVisible = isScrollable ? FIXED_VISIBLE - (canScrollUp ? 1 : 0) - (canScrollDown ? 1 : 0) : mm.filtered.length;
    mm._visibleItems = isScrollable ? FIXED_VISIBLE : mm.filtered.length;
    const hasGroups = mm.filtered.some((m) => m.group);
    let itemIdx = 0;
    let rendered = 0;
    let lastGroup = "";
    const listStartRow = row;
    ...
    for (let fi = 0; fi < mm.filtered.length; fi++) {
      const model = mm.filtered[fi];
      if (hasGroups && model.group && model.group !== lastGroup) {
        lastGroup = model.group;
        if (itemIdx >= scrollOff && rendered < itemsVisible) {
          this._write(ansi.cursorTo(row, 1) + ansi.eraseLine);
          const groupBadge = model.group === "Free" ? C.green(model.group) : model.group === "Go" ? C.cyan(model.group) : model.group === "Premium" ? C.yellow(model.group) : C.dim(model.group);
      ...
      const groupTag = model.group ? C.dim(` [${model.group}]`) : "";
      const descStr = model.desc ? C.dim("  " + model.desc) : "";
      const rawLen = 1 + 2 + labelStr.length + (model.group ? model.group.length + 3 : 0) + (model.desc ? model.desc.length + 2 : 0);
      const pad = Math.max(0, w - rawLen);
      const label = isHovered ? C.white.bold(labelStr) : C.white(labelStr);
      ...
      itemIdx++; row++; rendered++;
    }

    // Fill remaining rows in fixed container (keeps box height stable)
    if (isScrollable) {
      while (rendered < itemsVisible) {
        this._write(ansi.cursorTo(row, 1) + ansi.eraseLine);
        this._write(this._modalRow(startCol, w, " "));
    ...
    // Footer — border-top 0.5px #313244, fixed at bottom of container
    this._write(ansi.cursorTo(row, 1) + ansi.eraseLine);
  ...
  _drawToolProgress() {
    ...
    if (!tp._stableW) {
      const allLens = tp.tools.map((t) => 3 + t.name.length);
      tp._stableW = this._calcModalWidth([...allLens, 20], 6);
    }
    const w = tp._stableW;
    const startCol = Math.max(2, Math.floor((this.width - w) / 2) + 1);
    const startRow = this.contentRowStart;
    ...
    this._write(this._modalRow(startCol, w, " " + C.white.bold("TOOLS") + " " + C.dim(toolCountStr) + " ".repeat(Math.max(1, w - 7 - toolCountStr.length)) + C.dim("esc ")));
    row++;
    ...
    const isScrollable = tp.tools.length > FIXED_VISIBLE;
    const canScrollUp = isScrollable && tp.scrollOffset > 0;
    const canScrollDown = isScrollable && tp.scrollOffset + FIXED_VISIBLE < tp.tools.length;
    const itemsVisible = isScrollable ? FIXED_VISIBLE - (canScrollUp ? 1 : 0) - (canScrollDown ? 1 : 0) : tp.tools.length;
    const listStartRow = row;
    ...
    let rendered = 0;
    for (let i = tp.scrollOffset; i < tp.tools.length && rendered < itemsVisible; i++) {
      const tool = tp.tools[i];
      this._write(ansi.cursorTo(row, 1) + ansi.eraseLine);
    ...
    } else if (canScrollUp || tp.scrollOffset + itemsVisible < tp.tools.length) {
      // non-scrollable but still need down hint if overflow (fallback)
      if (tp.scrollOffset + itemsVisible < tp.tools.length) {
        this._write(ansi.cursorTo(row, 1) + ansi.eraseLine);
        this._write(this._modalRow(startCol, w, " " + C.dim("  \u25BC more below")));
        row++;
      }
  ...
  _drawSettingsModal() {
    ...
    // Stable width from all fields (HTML: 420px locked on open)
    if (!sm._stableW) {
    ...
    // Title bar
    this._write(ansi.cursorTo(row, 1) + ansi.eraseLine);
    const titlePad = Math.max(1, w - 1 - "SETTINGS".length - "esc ".length);
    this._write(this._modalRow(startCol, w, " " + C.pink.bold("SETTINGS") + " ".repeat(titlePad) + C.dim("esc ")));
    row++;
    ...
    for (let i = 0; i < sm.fields.length; i++) {
      ...
      let valStr;
      if (isEditing) {
        // FIX: render cursor within the edit buffer at the correct position
        const buf    = sm.editBuffer;
        const curPos = sm.editCursorPos;
        const before = buf.slice(0, curPos);
        const atCur  = buf[curPos] || " ";
        const after  = buf.slice(curPos + 1);
        valStr = C.white(before) + chalk.inverse(atCur) + C.white(after);
      } else if (f.masked) {
  ...
  _drawModal() {
    ...
    if (!m._stableW) {
      m._stableW = this._calcModalWidth([...m.lines.map((l) => l.length), m.title.length, 60], 6);
    }
    const w = m._stableW;
    ...
    for (let i = 0; i < maxVisible; i++) {
      ...
      this._write(ansi.cursorTo(r, 1) + ansi.eraseLine);
      const item = m.lines[i];
      if (!item) continue;
      const sel = i === m.selected;
      const bg  = sel ? chalk.bgHex("#313244") : chalk.bgHex("#1e1e2e");
      const inner = bg(" " + (sel ? C.pink("\u25B6 ") : "  ") + (sel ? C.white.bold(item) : C.dim(item)) + " ".repeat(Math.max(0, w - 3 - item.length)));
      this._write(this._modalRow(startCol, w, inner));
    }
      ...
      if (ch === "\x1B") {
        ...
        if (seq.startsWith("\x1B[<")) {
          ...
          if (m) {
            ...
            const eIdx = seq.indexOf("M", 3) !== -1 ? seq.indexOf("M", 3) + 1 : seq.indexOf("m", 3) + 1;
            i += eIdx; continue;
          }
        }
  ...
  _handleModelModalKey(data) {
    ...
    while (i < data.length) {
      ...
      if (ch === "\x01") { if (mm.onProvider) mm.onProvider(); i++; continue; }  // Ctrl+A
  ...
  // FIX: complete rewrite of settings modal key handler
  // — Ctrl+A, Ctrl+V, Ctrl+K, Ctrl+U, Ctrl+W now work
  // — arrow keys for cursor movement inside edit buffer
  // — input is fully isolated (never touches main inputBuffer)
    ...
    while (i < data.length) {
      ...
      if (ch === "\x1B") {
        ...
        if (seq.startsWith("\x1B[C")) {  // right arrow
          if (sm.editing >= 0) { sm.cursorRight(); this._dirty.settingsModal = true; this._dirty.input = true; this._renderDirty(); }
          i += 3; continue;
        }
        if (seq.startsWith("\x1B[D")) {  // left arrow
          if (sm.editing >= 0) { sm.cursorLeft(); this._dirty.settingsModal = true; this._dirty.input = true; this._renderDirty(); }
          i += 3; continue;
        }
        if (seq.startsWith("\x1B[H") || seq.startsWith("\x1B[1~")) {  // Home
          if (sm.editing >= 0) { sm.cursorHome(); this._dirty.settingsModal = true; this._dirty.input = true; this._renderDirty(); }
          i += seq.startsWith("\x1B[H") ? 3 : 4; continue;
        }
        if (seq.startsWith("\x1B[F") || seq.startsWith("\x1B[4~")) {  // End
          if (sm.editing >= 0) { sm.cursorEnd(); this._dirty.settingsModal = true; this._dirty.input = true; this._renderDirty(); }
          i += seq.startsWith("\x1B[F") ? 3 : 4; continue;
        }
        if (seq.startsWith("\x1B[3~")) {  // Delete key
          if (sm.editing >= 0) { sm.deleteChar(); this._dirty.settingsModal = true; this._dirty.input = true; this._renderDirty(); }
          i += 4; continue;
        }
      ...
      // Ctrl+A = move cursor to start of field (like shell)
      if (ch === "\x01") {
        if (sm.editing >= 0) { sm.cursorHome(); this._dirty.settingsModal = true; this._dirty.input = true; this._renderDirty(); }
        i++; continue;
      }
      ...
      if (ch === "\x0B") {
        if (sm.editing >= 0) { sm.killToEnd(); this._dirty.settingsModal = true; this._dirty.input = true; this._renderDirty(); }
        i++; continue;
      }

      // Ctrl+U = kill entire line
      if (ch === "\x15") {
        if (sm.editing >= 0) { sm.killLine(); this._dirty.settingsModal = true; this._dirty.input = true; this._renderDirty(); }
        i++; continue;
      }

      // Ctrl+W = delete word backwards
      if (ch === "\x17") {
        if (sm.editing >= 0) { sm.deleteWordBack(); this._dirty.settingsModal = true; this._dirty.input = true; this._renderDirty(); }
        i++; continue;
      }

      // Ctrl+V = paste from clipboard via PowerShell/pbpaste/xclip
      if (ch === "\x16") {
        if (sm.editing >= 0) {
          this._readClipboard((text) => {
            if (text) { sm.pasteText(text); this._dirty.settingsModal = true; this._dirty.input = true; this._renderDirty(); }
          });
        }
        i++; continue;
      }
      ...
      // Printable characters — only when editing
      if (ch >= " " && sm.editing >= 0) {
        sm.appendChar(ch);
        this._dirty.settingsModal = true; this._dirty.input = true; this._renderDirty();
        i++; continue;
  ...
  // FIX: clipboard read — works on Windows (PowerShell), macOS (pbpaste), Linux (xclip/xsel)
  _readClipboard(callback) {
    const { exec } = require("child_process");
    const platform = process.platform;
    let cmd;
    if (platform === "win32")  cmd = "powershell -NoProfile -Command Get-Clipboard";
    else if (platform === "darwin") cmd = "pbpaste";
  ...
  _processInput(data) {
    ...
    while (i < data.length) {
      ...
      // Ctrl+A = select all / home in main input
  ...
  _handleResize() {
    ...
    // FIX: reset stable widths on resize so they recompute for the new terminal size
