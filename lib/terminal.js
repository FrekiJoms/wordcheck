...
class Terminal {
  ...
  _drawContent() {
    const paletteRows = this.palette.visible ? Math.min(this.palette.filtered.length, 8) + 2 : 0;
  ...
  _drawPalette() {
    ...
    this._write(C.dim("  \u256D") + C.dim("\u2500".repeat(Math.min(this.width - 6, 52))) + C.dim("\u256E"));
  ...
  _scrollToTop()   { this.scrollOffset = 0; this._dirty.content = true; this._renderDirty(); }
  _scrollToBottom(){ this.scrollOffset = this.maxScroll; this._dirty.content = true; this._renderDirty(); }
