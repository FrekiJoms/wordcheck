...
class Terminal {
  ...
  _drawProviderModal() {
    ...
    // Calculate width from all items
    const contentLens = pm.filtered.map((e) => {
      if (e.sectionSep) return 2 + e.label.length;
      return 4 + (e.label || "").length + (e.badge ? e.badge.length + 1 : 0) + (e.desc ? e.desc.length + 2 : 0);
    });
    const entryCount = pm.entries ? pm.entries.length : pm.filtered.filter((r) => !r.sectionSep).length;
  ...
  _drawModelModal() {
    ...
    const contentLens = mm.filtered.map((m) => {
      return 4 + m.id.length + (m.group ? m.group.length + 3 : 0) + (m.desc ? m.desc.length + 2 : 0);
    });
    const titleLen = 7 + (mm.providerLabel ? mm.providerLabel.length + 2 : 0) + 8;
    const w        = this._calcModalWidth([...contentLens, titleLen, 22], 6);
    const startCol = Math.max(2, Math.floor((this.width - w) / 2) + 1);
    const startRow = this.contentRowStart;
