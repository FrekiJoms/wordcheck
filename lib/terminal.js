...
class Terminal {
  ...
  _drawContent() {
    // During scroll mode or when palette is hidden, use full content area
  ...
  _drawPalette() {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const desc = cmd.desc.length > descWidth ? cmd.desc.slice(0, descWidth) : cmd.desc.padEnd(descWidth);
