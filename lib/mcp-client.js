...
class MCPClient {
  ...
  /** Copy source file to a safe output path for modification */
  async copyForEdit(sourcePath) {
    const dir = path.dirname(sourcePath);
    const base = path.basename(sourcePath, ".docx");
    const outPath = path.join(dir, `${base}.wordcheck-fixed.docx`);
    await this.callTool("copy_document", {
      source_filename: sourcePath,
      destination_filename: outPath,
    });
    return outPath;
  }
