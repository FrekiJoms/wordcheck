...
class Agent {
  constructor() {
    ...
    this.sessions = new Map(); // id -> { id, title, filePath, scanResult, findings, conversationHistory, createdAt }
    this.activeSessionId = null;
    this.inputQueue = []; // queued prompts while AI is thinking
    this.isProcessing = false;
    this._loadSessions();
  }

  // -----------------------------------------------------------------------
  // Sessions — each scanned file = new session, title = doc basename else AI title
  // -----------------------------------------------------------------------
  _ensureSessionsDir() {
    try { fs.mkdirSync(Const.SESSIONS_DIR, { recursive: true }); } catch {}
  }

  _sessionTitleFromFile(filePath) {
    if (!filePath) return null;
    const base = path.basename(filePath);
    return base.replace(/\.docx$/i, "") || base;
  }
  ...
  _loadSessions() {
    try {
      ...
      const files = fs.readdirSync(Const.SESSIONS_DIR).filter((f) => f.endsWith(".json"));
      for (const f of files) {
        try {
          const data = JSON.parse(fs.readFileSync(path.join(Const.SESSIONS_DIR, f), "utf8"));
          if (data.id) this.sessions.set(data.id, data);
        } catch {}
      }
      if (this.sessions.size > 0 && !this.activeSessionId) {
        const sorted = [...this.sessions.values()].sort((a, b) => b.createdAt - a.createdAt);
        this.activeSessionId = sorted[0].id;
        const cur = this.sessions.get(this.activeSessionId);
        if (cur) {
          this.filePath = cur.filePath;
          this.scanResult = cur.scanResult;
          this.findings = cur.findings || [];
          this.conversationHistory = cur.conversationHistory || [{ role: "system", content: SYSTEM_PROMPT }];
        }
      }
    } catch {}
  }
  ...
  _persistSession(id) {
    try {
      ...
      if (!s) return;
      fs.writeFileSync(path.join(Const.SESSIONS_DIR, id + ".json"), JSON.stringify(s, null, 2), "utf8");
    } catch {}
  }

  _createSession({ filePath, title, scanResult, findings }) {
    const id = this._generateSessionId();
    const derivedTitle = title || this._sessionTitleFromFile(filePath) || `Untitled Chat ${new Date().toLocaleDateString()}`;
    const session = {
      id,
      title: derivedTitle,
      filePath: filePath || null,
      scanResult: scanResult || null,
      findings: findings ? JSON.parse(JSON.stringify(findings)) : [],
      conversationHistory: [{ role: "system", content: SYSTEM_PROMPT + `\n\nCurrent session: ${derivedTitle} (${id})` }],
      createdAt: Date.now(),
    };
  ...
  _switchSession(id) {
    ...
    if (this.activeSessionId) {
      const cur = this.sessions.get(this.activeSessionId);
      if (cur) {
        cur.scanResult = this.scanResult;
        cur.findings = this.findings;
        cur.conversationHistory = this.conversationHistory;
        cur.filePath = this.filePath;
        this._persistSession(this.activeSessionId);
      }
    }
    this.activeSessionId = id;
    this.filePath = s.filePath;
    this.scanResult = s.scanResult;
    this.findings = s.findings || [];
    this.conversationHistory = s.conversationHistory || [{ role: "system", content: SYSTEM_PROMPT }];
    this._updateHeader();
    // Restore content view: clear and re-render findings if any
    this.terminal.clearContent();
    if (this.findings.length) this._renderFindingsTable();
    else this.terminal.addLine(C.dim("  Switched to session: ") + C.white.bold(s.title) + (s.filePath ? C.dim(" — " + path.basename(s.filePath)) : ""));
    return s;
  }
  ...
  _toolListSessions() {
    const list = [...this.sessions.values()].map((s) => ({
      id: s.id,
      title: s.title,
      file: s.filePath ? path.basename(s.filePath) : "(no file)",
      createdAt: new Date(s.createdAt).toLocaleString(),
      active: s.id === this.activeSessionId,
      findings: (s.findings || []).length,
    }));
    return { sessions: list, active: this.activeSessionId };
  }

  _toolSwitchSession(id) {
    const s = this._switchSession(id);
    if (!s) return { error: "Session not found: " + id };
    return { switched: s.id, title: s.title, file: s.filePath };
  }

  _toolCreateSession(title, filePath) {
    // Guard: ask before creating multiple sessions in same turn is handled via SYSTEM_PROMPT,
    // but we also enforce: if title not provided and filePath not provided, generate.
    const effectiveTitle = title || this._sessionTitleFromFile(filePath) || `Untitled Chat ${new Date().toLocaleDateString()}`;
    const s = this._createSession({ filePath: filePath || null, title: effectiveTitle, scanResult: null, findings: [] });
    // Switch to it (already active)
    this.terminal.clearContent();
    this.terminal.addLine(C.green("  \u2713 Created session: ") + C.white.bold(s.title) + C.dim(` (${s.id})`));
    return { created: s.id, title: s.title, file: s.filePath };
  }

  async _openSessionModal() {
    const sessions = [...this.sessions.values()].sort((a, b) => b.createdAt - a.createdAt).map((s) => ({
      id: s.id,
      title: s.title,
      file: s.filePath ? path.basename(s.filePath) : "",
      createdAt: s.createdAt,
    }));
    this.terminal.openSessionModal({
      sessions,
      activeId: this.activeSessionId,
      onSelect: (id) => {
        this.terminal.closeSessionModal();
        const s = this._switchSession(id);
        if (s) this.terminal.addLine(C.green("  \u2713 Loaded session: ") + C.white.bold(s.title));
      },
      onCancel: () => this.terminal.closeSessionModal(),
      onCreate: () => {
        this.terminal.closeSessionModal();
        const title = `Untitled Chat ${new Date().toLocaleDateString()}`;
        this._toolCreateSession(title, null);
      },
    });
  }
  ...
  async repl() {
    while (true) {
      let input;
      // Prioritize queued inputs typed while AI was thinking (sticky bottom)
      if (this.terminal.hasQueued()) {
        input = this.terminal.dequeueInput();
        this.terminal.setStatus(`queued:${this.terminal.inputQueue.length} | ready`);
      } else {
        input = await this.terminal.prompt();
      }
      if (input === null) break;
      ...
      if (this.llmConnected) {
        this.terminal.setAIThinking(true);
        this.isProcessing = true;
        try {
          await this._chatWithAI(cmd);
        } finally {
          this.isProcessing = false;
          this.terminal.setAIThinking(false);
          // Persist active session's history
          if (this.activeSessionId) this._persistSession(this.activeSessionId);
        }
        // Loop will check queue at top and process next without waiting for prompt
        continue;
      } else {
  ...
  async _executeTool(name, args) {
    const context = {
      ...
      terminal: this.terminal, agent: this,
  ...
  _renderToolResult(name, args, result) {
    ...
    switch (name) {
      case "scan_document":
        if (result.path) { this._updateHeader(); this.terminal.addLine(); this._renderFindingsTable(); this.terminal.setStatus(path.basename(this.filePath)); }
        break;
      case "get_findings":
  ...
  async _scanDocument(filePath) {
    ...
    // Create new session per scanned file — title = document basename
    const session = this._createSession({ filePath, scanResult: this.scanResult, findings: this.findings });
    this.terminal.addLine(C.dim("  Session: ") + C.white.bold(session.title) + C.dim(` (${session.id})`));
    this._updateHeader();
    this.terminal.clearContent();
  ...
  _printHelp() {
    const cmds = [
      ...
      ["/session","manage sessions (load previous)"],
