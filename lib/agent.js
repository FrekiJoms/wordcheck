...
class Agent {
  constructor() {
    ...
    this._modelDropdownActive = false;
  }
  ...
  async repl() {
    while (true) {
      ...
      // Route to settings input handler if active
      if (this._settingsMode) {
        this.terminal.addLine();
        this.terminal.addLine(C.bar("  │ ") + C.pink.bold("you") + C.dim(" › ") + C.white(cmd));
        this._handleSettingsInput(cmd);
        continue;
      }

      // Route to model dropdown handler if active
      if (this._modelDropdownActive) {
        this.terminal.addLine();
        this.terminal.addLine(C.bar("  │ ") + C.pink.bold("you") + C.dim(" › ") + C.white(cmd));
        this._handleModelDropdown(cmd);
        continue;
      }
  ...
  // -----------------------------------------------------------------------
  // Settings — displayed as content lines, input handled via REPL
  // -----------------------------------------------------------------------
  _openSettings() {
    this.terminal.clearContent();
    const api = this.config.api;
    const maskedKey = api.apiKey ? "*".repeat(Math.min(api.apiKey.length, 16)) + "..." : "(not set)";
    ...
    const lines = fields.map((f, i) =>
      C.cyan("  " + String(i + 1) + ". ") + C.white.bold(f.label.padEnd(14)) + C.dim(f.value)
    );

    lines.push("");
    lines.push(C.dim("  Type a number (1-6) to edit that field"));
    lines.push(C.dim("  Press Esc or type anything else to close"));

    const panel = renderPanel({
      title: C.pink.bold("SETTINGS"),
      lines,
      width: this.terminal.width - 4,
      dimStyle: C.bar,
    });
  ...
  _handleSettingsInput(input) {
    const trimmed = input.trim();

    // If we're waiting for a new value
    if (this._editField) {
      this._applySettingsValue(this._editField, trimmed);
      this._editField = null;
      this._settingsMode = false;
      return true;
    }

    // If we're waiting for a field number
    const num = parseInt(trimmed, 10);
    if (num >= 1 && num <= 6) {
      const fieldKeys = ["provider", "baseUrl", "model", "apiKey", "maxTokens", "temperature"];
      this._showEditPrompt(fieldKeys[num - 1]);
      return true;
    }

    // Anything else closes settings
    this.terminal.addLine(C.dim("  Settings closed."));
    this._settingsMode = false;
    return true;
  }

  _showEditPrompt(field) {
    const api = this.config.api;
    const current = api[field];
    const masked = field === "apiKey" ? "*".repeat(Math.min((current || "").length, 16)) + "..." : current;

    this.terminal.addLine();
    this.terminal.addLine(C.bar("  │ ") + C.pink.bold("Editing: ") + C.white.bold(field));
    ...
    if (field === "provider") {
      ...
      for (let i = 0; i < providers.length; i++) {
        ...
        this.terminal.addLine(
          C.cyan("  " + (i + 1) + ". ") +
          (isCurrent ? C.white.bold(p + "  (active)") : C.dim(p))
        );
      }
      this.terminal.addLine(C.dim("  Type provider name or number"));
    } else if (field === "model") {
      this._openModelDropdown();
      return;
    } else if (field === "apiKey") {
      this.terminal.addLine(C.dim("  Enter new API key:"));
    } else if (field === "maxTokens") {
      this.terminal.addLine(C.dim("  Enter new value (current: " + api.maxTokens + "):"));
    } else if (field === "temperature") {
      this.terminal.addLine(C.dim("  Enter new value (0.0-2.0, current: " + api.temperature + "):"));
    } else if (field === "baseUrl") {
      this.terminal.addLine(C.dim("  Enter new URL:"));
    }
  ...
  _applySettingsValue(field, value) {
    const api = this.config.api;

    if (field === "provider") {
      const providers = ["opencode-go", "openai", "ollama", "custom"];
      const num = parseInt(value, 10);
      const val = (num >= 1 && num <= providers.length) ? providers[num - 1] : value;
      if (val && PROVIDER_DEFAULTS[val]) {
        ...
        this.terminal.addLine(C.red("  \u2717 Invalid provider. Try: opencode-go, openai, ollama, custom"));
      }
    } else if (field === "apiKey") {
      if (!value) {
  ...
  // -----------------------------------------------------------------------
  // Model dropdown — uses palette like / commands
  // -----------------------------------------------------------------------
  _openModelDropdown() {
    ...
    this.terminal.palette.commands = models.map((m) => ({
      cmd: "/model " + m.id,
      desc: m.id === current ? m.desc + " (active)" : m.desc,
    }));
    this.terminal.palette.filtered = this.terminal.palette.commands;
    this.terminal.palette.selected = models.findIndex((m) => m.id === current) || 0;
    this.terminal.palette.visible = true;
    this.terminal._dirty.palette = true;
    this.terminal._dirty.content = true;
    this.terminal._renderDirty();

    // Intercept the next input to handle model selection
    this._modelDropdownActive = true;
  }

  _handleModelDropdown(input) {
    const trimmed = input.trim();
    // Extract model ID from "/model <id>" format
    ...
    if (match) {
      ...
      this.terminal.addLine(C.green("  \u2713 Model: ") + C.white.bold(modelId) +
        (found ? C.dim("  " + found.desc) : ""));
      this.terminal.addLine(C.dim("  Provider: ") + C.white(provider) +
        C.dim("  Base URL: ") + C.white(this.config.api.baseUrl));
      this._updateHeader();
    } else {
      this.terminal.addLine(C.dim("  Cancelled."));
    }
    // Restore palette to default commands
    this.terminal.palette.commands = DEFAULT_COMMANDS;
    this.terminal.palette.visible = false;
    this._modelDropdownActive = false;
