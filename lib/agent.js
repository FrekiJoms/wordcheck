...
// ---------------------------------------------------------------------------
// Hardcoded provider presets (always available, user can override)
// ---------------------------------------------------------------------------
const BUILTIN_PROVIDERS = [
  ...
  },
  {
    id: "opencode-go",
    label: "OpenCode Go",
    badge: "Low cost subscription for everyone",
    desc: "",
    baseUrl: "https://opencode.ai/zen/go/v1",
    defaultModel: "mimo-v2.5-pro",
  },
  {
    id: "openai",
    label: "OpenAI",
    badge: "(ChatGPT Plus/Pro or API key)",
    desc: "",
...
const POPULAR_PROVIDERS  = ["opencode-zen", "opencode-go", "openai", "github-copilot", "anthropic", "google"];
...
function allProviders() {
  return [...BUILTIN_PROVIDERS, ...EXTRA_PROVIDERS];
}
...
class Agent {
  ...
  // -----------------------------------------------------------------------
  // Provider modal — OpenCode-style picker
  // -----------------------------------------------------------------------
  _openProviderModal() {
  ...
  _applyProvider(providerId) {
    ...
    // If it needs an API key, auto-open settings
    if (["openai", "anthropic", "google"].includes(providerId)) {
      setTimeout(() => this._openSettingsModal(), 200);
    }
  }
  ...
  _openSettingsModal() {
    ...
    this.terminal.openSettingsModal({
      ...
      onSave: (key, value) => {
        ...
        // Update the field display value
  ...
  async _chatWithAI(userMessage) {
    ...
    try {
      ...
      if (response.toolCalls && response.toolCalls.length > 0) {
        ...
        if (finalResponse.content) this._printAIResponse(finalResponse.content);
  ...
  _renderToolCallBlock(lines) {
    ...
    const icons = { scan_document:"\u2609",create_document:"\u270E",open_document:"\u27A4",get_document_text:"\u2630",get_paragraph:"\u2630",search_replace:"\u21C4",add_paragraph:"\u2795",add_heading:"\u2795",delete_paragraph:"\u2716",format_text:"\u2592",add_table:"\u25A6",convert_to_pdf:"\u27A4",copy_document:"\u2398",approve_finding:"\u2713",fix_approved:"\u2692",list_documents:"\u2630",get_document_info:"\u2139",get_document_outline:"\u2630",find_text_in_document:"\u2315" };
  ...
  _printHelp() {
    const cmds = [
      ...
      ["/settings","configure AI settings (or /modal)"],["/model","change AI provider"],
      ["/open","open document in Word"],["/file","show current file path"],
    ...
    const panel = renderPanel({ title: C.pink.bold("COMMANDS"), lines: [
      ...
      C.dim("Tip: type /settings or /modal then Tab to open the settings modal."),
