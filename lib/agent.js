...
class Agent {
  ...
  _printAIResponse(text) {
    ...
    for (const line of lines) {
      ...
      // --- Horizontal rule ---
      if (/^[-*_]{3,}$/.test(trimmed)) {
        this.terminal.addLine(C.dim("  " + "\u2500".repeat(50)));
        continue;
      }
  ...
  _showFixDiff(original, modified) {
    ...
    const diffLines = renderSideBySideDiff(cleanOrig, cleanMod, {
      width: Math.min(this.terminal.width - 4, 100),
    });
      ...
      case "get_document_text":
        if (result.text) {
          ...
          for (const line of preview) {
            this.terminal.addLine("    " + this._renderInline(line.slice(0, 70)));
          }
          if (clean.length > 500) {
        ...
        if (result.text) {
          ...
          for (const w of words) {
            if ((line + " " + w).trim().length > 64 && line) {
              this.terminal.addLine("    " + this._renderInline(line.trim()));
              line = w;
            } else { line = line ? line + " " + w : w; }
          }
          if (line) this.terminal.addLine("    " + this._renderInline(line.trim()));
        }
        break;
