...
// ---------------------------------------------------------------------------
// AI phrase dictionary — weights are calibrated heuristics, do not change
...
// Pre-compile regexes at module load — avoids recompiling on every paragraph.
...
// ---------------------------------------------------------------------------
// Replacement suggestions — ordered by preference
// ---------------------------------------------------------------------------
const REPLACEMENTS = {
...
// ---------------------------------------------------------------------------
// Core scoring — do not alter weights without re-validation
// ---------------------------------------------------------------------------
function scoreParagraph(text, idx) {
  ...
  // 1. AI phrase matching — uses pre-compiled regexes
  for (const [phrase, { weight, re }] of Object.entries(AI_PHRASE_REGEXES)) {
    // Reset lastIndex since we share compiled regex objects across calls
    re.lastIndex = 0;
...
// ---------------------------------------------------------------------------
// DOCX scanning — single mammoth pass (HTML only; raw text removed)
// ---------------------------------------------------------------------------
async function scanDisk(filePath) {
  const mammoth = require("mammoth");

  // Single pass: HTML gives us structure (bold detection) AND text content.
  // The previous extractRawText() call was redundant and has been removed.
...
// ---------------------------------------------------------------------------
// Fix suggestions — uses pre-computed flag data where possible to avoid
// redundant sentence-starter recomputation.
// ---------------------------------------------------------------------------
function suggestFixes(para) {
  const suggestions = [];
  const lower = para.text.toLowerCase();

  // Phrase replacements — derived from flags already computed in scoreParagraph
  ...
  // Reuse starter flag data rather than recomputing mostCommon on sentences
